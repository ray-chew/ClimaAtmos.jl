...
"""
    compute_Φ_r_θ_vr(params, thermo_params)

    Compute the reference geopotential and virtual potential temperature profiles to be used in the pressure gradient update.
"""
function compute_Φ_r_θ_vr!(Π, Φ_r, θ_vr, params, ᶜts)
    thermo_params = CAP.thermodynamics_params(params)
